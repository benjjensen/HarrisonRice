...
struct GuiElements {
     ...
     *
     * This needs to be a text view and not an entry because text view widgets
     * can accept multiline entries, where as entry widgets accept only a single
     * line.
     */
    GtkWidget* edit_notes_text_view;
    /**
     * The label in the edit notes window where we display problems with the
     * input in edit_notes_text_view.
     */
...
struct SamplerState {
    ...
    /**
     * The process id for the child process where the acquire program is
     * running.
     */
    int acquire_process_id;
    /**
     * The file descriptor for the output end of the pipe that the acquire
     * program is feeding with its output.
     */
    int acquire_output_fd;

    /**
     * The index of the capture in finished_captures for which the user is
     * currently editing the notes.
     */
...
static bool validate_new_option_and_trim(SamplerState& state,
        std::string &new_option_code, std::string &new_option_name);
...
static void init_main_window(SamplerState& state) {
    ...
    gtk_container_set_border_width(GTK_CONTAINER(gui.main_window),
            WINDOW_BORDER_WIDTH);
    gtk_window_set_position(GTK_WINDOW(gui.main_window),
    ...
    gtk_scrolled_window_set_policy(
            GTK_SCROLLED_WINDOW(gui.capture_table_scroll_window),
            GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_box_pack_start(GTK_BOX(layout_box), gui.capture_table_scroll_window,
            TRUE, TRUE, NO_PADDING);

    // 0, 0, 0, 0 means that what's inside of this alignment widget will not
    ...
    gtk_viewport_set_shadow_type(GTK_VIEWPORT(gtk_bin_get_child(
            GTK_BIN(gui.capture_table_scroll_window))), GTK_SHADOW_NONE);

    gui.capture_table_current_rows = 1;
    gui.capture_table = gtk_table_new(gui.capture_table_current_rows,
            COLS_COUNT, FALSE);
    ...
    gtk_widget_show(label);
    gtk_table_attach_defaults(GTK_TABLE(gui.capture_table), label, COL_NOTES,
            COL_NOTES + 1, row, row + 1);
    ...
    // This box contains the labels that display information about the current
    // captures.
    gui.current_capture_box = gtk_hbox_new(FALSE, NO_PADDING);
    gtk_box_pack_start(GTK_BOX(footer_box), gui.current_capture_box, FALSE,
            FALSE, NO_PADDING);
    ...
    gtk_box_pack_start(GTK_BOX(gui.current_capture_box),
            gui.current_capture_status, FALSE, FALSE, READY_BUTTON_PADDING);
    gtk_widget_show(gui.current_capture_status);

    // This label tells the user when there's an error saving the captured data
    // to the disk.
    gui.label_capture_error = gtk_label_new(
            "ERROR: Problem saving data. Is the disk full?");
    GdkColor color;
    gdk_color_parse("red", &color);
    // Set the error label's text color to red:
    gtk_widget_modify_fg(gui.label_capture_error, GTK_STATE_NORMAL, &color);
    gtk_box_pack_start(GTK_BOX(footer_box), gui.label_capture_error, FALSE,
            FALSE, NO_PADDING);
    // Intentionally don't show this yet.

    gui.label_total_data_captured = gtk_label_new("0 GB captured in total");
    gtk_box_pack_end(GTK_BOX(footer_box), gui.label_total_data_captured,
    ...
    gtk_box_pack_start(GTK_BOX(sub_box), gui.new_button, TRUE, TRUE,
            NO_PADDING);
    ...
    gtk_widget_set_size_request(gui.reset_button, MINOR_BUTTON_MIN_WIDTH,
            DEFAULT_HEIGHT);
    gtk_box_pack_start(GTK_BOX(sub_box), gui.reset_button, FALSE, FALSE,
            NO_PADDING);
    ...
    gtk_box_pack_start(GTK_BOX(sub_box), gui.start_button, TRUE, TRUE,
            NO_PADDING);
    ...
    gtk_box_pack_start(GTK_BOX(sub_box), gui.stop_button, TRUE, TRUE,
            NO_PADDING);
    ...
    // Populate the list with the captures that were loaded from the files in
    // the data folder.
    // finished_captures should already have been initialized by now.
    for(std::vector<DataCapture>::iterator it = state.finished_captures.begin();
            it != state.finished_captures.end(); ++it) {
...
static void init_new_capture_window(SamplerState& state) {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(gui.new_capture_window),
            GTK_WINDOW(gui.main_window));
    gtk_window_set_title(GTK_WINDOW(gui.new_capture_window), "New Capture");
    gtk_container_set_border_width(GTK_CONTAINER(gui.new_capture_window),
            WINDOW_BORDER_WIDTH);
    gtk_window_set_position(GTK_WINDOW(gui.new_capture_window),
            GTK_WIN_POS_CENTER_ALWAYS);
    ...
    gtk_box_pack_start(GTK_BOX(layout_box), gui.fields_parent_box, FALSE, FALSE,
            NO_PADDING);

    // Call the separate function that populates the fields into
    // fields_parent_box:
    ...
    // it can begin capturing.
    GtkWidget* create_new_capture_button = gtk_button_new_with_label("Prepare");
    gtk_box_pack_start(GTK_BOX(sub_box), create_new_capture_button, TRUE, TRUE,
            NO_PADDING);
    g_signal_connect(create_new_capture_button, "clicked",
            G_CALLBACK(cb_create_new_capture), (gpointer)(&state));
    gtk_widget_show(create_new_capture_button);

    // The cancel new capture button cancels making a new capture and returns to
    // the main window.
    GtkWidget* cancel_new_capture_button = gtk_button_new_with_label("Cancel");
    gtk_widget_set_size_request(cancel_new_capture_button,
            MINOR_BUTTON_MIN_WIDTH, DEFAULT_HEIGHT);
    gtk_box_pack_start(GTK_BOX(sub_box), cancel_new_capture_button, FALSE,
            FALSE, NO_PADDING);
    g_signal_connect(cancel_new_capture_button, "clicked",
            G_CALLBACK(cb_cancel_new_capture), (gpointer)(&state));
...
static void init_add_option_window(SamplerState& state) {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(gui.add_option_window),
            GTK_WINDOW(gui.new_capture_window));
    gtk_window_set_title(GTK_WINDOW(gui.add_option_window), "Add Option");
    gtk_container_set_border_width(GTK_CONTAINER(gui.add_option_window),
            WINDOW_BORDER_WIDTH);
    gtk_window_set_position(GTK_WINDOW(gui.add_option_window),
            GTK_WIN_POS_CENTER_ALWAYS);
    ...
    gtk_box_pack_start(GTK_BOX(layout_box), gui.add_option_label_error, FALSE,
            FALSE, NO_PADDING);
    ...
    gtk_box_pack_start(GTK_BOX(sub_box), add_option_add_button, TRUE, TRUE,
            NO_PADDING);
    g_signal_connect(add_option_add_button, "clicked",
            G_CALLBACK(cb_add_option_add), (gpointer)(&state));
    ...
    gtk_box_pack_start(GTK_BOX(sub_box), add_option_cancel_button, FALSE, FALSE,
            NO_PADDING);
    gtk_widget_set_size_request(add_option_cancel_button,
            MINOR_BUTTON_MIN_WIDTH, DEFAULT_HEIGHT);
    g_signal_connect(add_option_cancel_button, "clicked",
            G_CALLBACK(cb_add_option_cancel), (gpointer)(&state));
...
static void init_edit_notes_window(SamplerState& state) {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(gui.edit_notes_window),
            GTK_WINDOW(gui.main_window));
    gtk_window_set_title(GTK_WINDOW(gui.edit_notes_window), "Edit notes");
    gtk_container_set_border_width(GTK_CONTAINER(gui.edit_notes_window),
            WINDOW_BORDER_WIDTH);
    gtk_window_set_position(GTK_WINDOW(gui.edit_notes_window),
            GTK_WIN_POS_CENTER_ALWAYS);
    // The user can't close this window except through our GUI:
    ...
    gtk_box_pack_start(GTK_BOX(layout_box), gui.edit_notes_label_error, FALSE,
            FALSE, NO_PADDING);
    ...
    g_signal_connect(save_button, "clicked", G_CALLBACK(cb_edit_notes_save),
            (gpointer)(&state));
    gtk_widget_show(save_button);

    // This button cancels editing the notes:
    GtkWidget* cancel_button = gtk_button_new_with_label("Cancel");
    gtk_box_pack_start(GTK_BOX(sub_box), cancel_button, FALSE, FALSE,
            NO_PADDING);
    gtk_widget_set_size_request(cancel_button, MINOR_BUTTON_MIN_WIDTH,
            DEFAULT_HEIGHT);
    g_signal_connect(cancel_button, "clicked", G_CALLBACK(cb_edit_notes_cancel),
            (gpointer)(&state));
...
static void populate_fields_and_options(SamplerState& state) {
    ...
    std::list<std::string> field_order =
            state.naming_convention.get_field_order();
    for(std::list<std::string>::iterator field_it = field_order.begin();
        ...
        gtk_box_pack_start(GTK_BOX(state.gui.fields_parent_box), field_box,
                TRUE, TRUE, NO_PADDING);

        std::string field_code = *field_it;
        // Store the code for the field in the field_box object so that it can
        // be retrieved later by cb_field_selected_changed:
        gtk_widget_set_name(field_box, field_code.c_str());

        // This label shows the name of the field:
        GtkWidget* field_label = gtk_label_new(
                state.naming_convention.get_field_name(field_code).c_str());
        gtk_box_pack_start(GTK_BOX(field_box), field_label, FALSE, FALSE,
                NO_PADDING);
        gtk_widget_show(field_label);

        std::string selected_option =
                state.naming_convention.get_field_value(field_code);

        GtkWidget* option_radio_button = NULL;
        std::map<std::string, std::string> options =
                state.naming_convention.get_options(field_code);
        for(std::map<std::string, std::string>::iterator option_it =
                options.begin(); option_it != options.end(); ++option_it) {
            ...
            if(option_code == selected_option) {
                // If this is the selected option, set this as the active radio
                // button.
            ...
            // Store the code for this option in the radio button widget so that
            // it can be retrieved by cb_field_selection_changed.
            gtk_widget_set_name(option_radio_button, option_code.c_str());
            g_signal_connect(option_radio_button, "toggled",
                    G_CALLBACK(cb_field_selection_changed), (gpointer)(&state));
        ...
        gtk_widget_set_size_request(add_option_button, MINOR_BUTTON_MIN_WIDTH,
                DEFAULT_HEIGHT);
        g_signal_connect(add_option_button, "clicked",
                G_CALLBACK(cb_add_new_option), (gpointer)(&state));

        // Use this alignment widget to prevent the button from expanding (so
...
static void insert_capture_into_table(SamplerState& state,
        DataCapture capture) {
    // Add a row to the table:
    gtk_table_resize(GTK_TABLE(state.gui.capture_table),
            ++state.gui.capture_table_current_rows, COLS_COUNT);
    // Put the capture into that row:
    insert_capture_into_table_row(state, capture,
            state.gui.capture_table_current_rows - 1);
}
...
static void insert_capture_into_table_row(SamplerState& state,
    ...
    gtk_widget_show(label);
    gtk_table_attach_defaults(GTK_TABLE(state.gui.capture_table), label,
            COL_NAME, COL_NAME + 1, row, row + 1);
    ...
    gtk_widget_show(label);
    gtk_table_attach_defaults(GTK_TABLE(state.gui.capture_table), label,
            COL_DURATION, COL_DURATION + 1, row, row + 1);
    ...
    gtk_widget_show(label);
    gtk_table_attach_defaults(GTK_TABLE(state.gui.capture_table), label,
            COL_NOTES, COL_NOTES + 1, row, row + 1);
    ...
    g_signal_connect(edit_notes_button, "clicked", G_CALLBACK(cb_edit_notes),
            (gpointer)(&state));
    ...
    g_signal_connect(delete_capture_button, "clicked",
            G_CALLBACK(cb_delete_capture), (gpointer)(&state));
    ...
    gtk_widget_show(align);
    gtk_table_attach_defaults(GTK_TABLE(state.gui.capture_table), align,
            COL_DELETE_CAPTURE, COL_DELETE_CAPTURE + 1, row, row + 1);
    ...
    gtk_scrolled_window_set_vadjustment(GTK_SCROLLED_WINDOW(
            state.gui.capture_table_scroll_window), adjustment);
}
...
static void hide_capture_from_table(GuiElements& gui, const int capture_index) {
    ...
    for(iter = children; iter != NULL; iter = g_list_next(iter)) {
        ...
        gtk_container_child_get(GTK_CONTAINER(gui.capture_table), child,
...
static void cb_start_capture(GtkWidget* widget, gpointer data) {
    ...
    gtk_label_set_text(GTK_LABEL(state.gui.current_capture_status),
            "capturing data... (click stop to finish)");
...
static void cb_new_capture(GtkWidget* widget, gpointer data) {
    ...
    // Setting the new capture window as modal prevents other windows in the
    // program from receiving focus (so the only window that will accept input
    // is the new capture window).
...
static void cb_create_new_capture(GtkWidget* widget, gpointer data) {
    ...
    // The input from the user in the radio group for each field in the naming
    // convention is already in the naming_convention object because it's stored
    // there by cb_field_selection_changed whenever the user clicks on a radio
    // button. So, here, all we have to do is have the naming convention build
    // the name:
    state.current_capture.name = state.naming_convention.name();

    // Display to the user the name of the current capture and that it's ready
    // to begin:
    gtk_label_set_text(GTK_LABEL(state.gui.current_capture_name),
            state.current_capture.name.c_str());
    gtk_label_set_text(GTK_LABEL(state.gui.current_capture_status),
            "ready to capture");
    gtk_widget_show(state.gui.current_capture_box);

    // At this point, the user can either cancel the current capture or start
    // the current capture, so those are the buttons that are active right now:
...
static void cb_add_new_option(GtkWidget* widget, gpointer data) {
    ...
    // The field code was stored in the add option button by
    // populate_fields_and_options:
    state.field_being_extended = gtk_widget_get_name(widget);

    // Show the user the name of the current field being extended and clear the
    // entry fields:
    gtk_label_set_text(GTK_LABEL(state.gui.add_option_field_label),
            ("  " + state.naming_convention.get_field_name(
            state.field_being_extended)).c_str());
...
static void cb_field_selection_changed(GtkWidget* widget, gpointer data) {
    ...
    // widget is the radio button that was clicked. Its name was assigned to the
    // code of the option in populate_fields_and_options. The parent of widget
    // is the field box, and its name was assigned to the code of the field in
    // populate_fields_and_options.
    std::string field_code = gtk_widget_get_name(gtk_widget_get_parent(widget));
    std::string option_code = gtk_widget_get_name(widget);

    // Store the user's selection in naming_convention so that when we need to
    // get the chosen name, we don't have to crawl over the radio buttons to
...
static void cb_add_option_add(GtkWidget* widget, gpointer data) {
    ...
    std::string new_option_code =
            gtk_entry_get_text(GTK_ENTRY(state.gui.add_option_entry_code));
    std::string new_option_name =
            gtk_entry_get_text(GTK_ENTRY(state.gui.add_option_entry_name));

    if(!validate_new_option_and_trim(state, new_option_code, new_option_name)) {
        // The new option is not valid, so we can't do anything here. The user
        // was informed by the validate_new_option_and_trim function, so we
        // don't need to worry about that here. Just return from this function
        // and the user will have another chance to change their input to be
        // valid.
        return;
    }

    state.naming_convention.add_option(state.field_being_extended,
            new_option_code, new_option_name);
    // Re-populate the fields and option radio buttons now that there's a new
    // option.
    populate_fields_and_options(state);
...
static void cb_edit_notes(GtkWidget* widget, gpointer data) {
    ...
    gtk_window_set_gravity(GTK_WINDOW(state.gui.edit_notes_window),
            GDK_GRAVITY_CENTER);
...
static void cb_edit_notes_save(GtkWidget* widget, gpointer data) {
    ...
    std::string new_notes = gtk_text_buffer_get_text(buffer, &start_iter,
            &end_iter, FALSE);

    if(new_notes.find('`') != std::string::npos) {
        // Backquote characters are not allowed in the notes text. Tell the
        // user.
        gtk_label_set_text(GTK_LABEL(state.gui.edit_notes_label_error),
                "Backquote (`) character not allowed");
        return;
    }
    ...
    gtk_label_set_text(
            GTK_LABEL(state.gui.notes_labels[state.notes_being_edited]),
            new_notes.c_str());
...
static bool validate_new_option_and_trim(SamplerState& state,
        std::string &new_option_code, std::string &new_option_name) {
    // Empty strings and strings that are only space characters are not allowed
    // for either the name or the code.
    if(new_option_code == "" || new_option_name == "" ||
            new_option_code.find_first_not_of(' ') == std::string::npos ||
            new_option_name.find_first_not_of(' ') == std::string::npos) {
        gtk_label_set_text(GTK_LABEL(state.gui.add_option_label_error),
                "No empty parameters allowed");
        return FALSE;
    }
    // At this point, neither string is empty, nor is either string only space
    // characters.

    // Trim the whitespace off of option code.
    if(new_option_code.find(' ') != -1) {
        size_t first_not_space = new_option_code.find_first_not_of(' ');
        // This is safe because we know that the string is not only space
        // characters.
        new_option_code = new_option_code.substr(first_not_space,
                new_option_code.find_last_not_of(' ') - first_not_space + 1);
    }
    ...
    if(new_option_name.find(' ') != -1) {
        ...
        // This is safe because we know that the string is not only space
        // characters.
        new_option_name = new_option_name.substr(first_not_space,
                new_option_name.find_last_not_of(' ') - first_not_space + 1);
    }

    if(new_option_code.find(' ') != -1) {
        // After stripping the whitespace off of the option code, if it still
        // has spaces in it, it's invalid:
        gtk_label_set_text(GTK_LABEL(state.gui.add_option_label_error),
                "No spaces allowed in code");
        return FALSE;
    }

    // Check each currently existing option in the field being extended to see
    // if the new option shares a name or a code with it. If it does, the new
    // option is not valid.
    std::map<std::string, std::string> current_options =
            state.naming_convention.get_options(state.field_being_extended);
    for(std::map<std::string, std::string>::iterator option_it =
            current_options.begin(); option_it != current_options.end();
            ++option_it) {
        std::string option_code = option_it->first;
        std::string option_name = option_it->second;
        ...
        if(option_code == new_option_code) {
            gtk_label_set_text(GTK_LABEL(state.gui.add_option_label_error),
                    ("This code already in use for " + option_name).c_str());
            return FALSE;
        }
...
/**
 * This function returns true if the first DataCapture happened before the
 * second one, false otherwise.
 */
static bool compare_captures(const DataCapture &first,
        const DataCapture &second) {
...
static void load_all_captures_from_files(SamplerState& state) {
...

    {
        ...
        if(directory_pointer) {
            while((item = readdir(directory_pointer)) != NULL) {
                ...
                int expected_index = filename.size() -
                        CAPTURE_META_ENDING.size();
                if(found_index != std::string::npos &&
                        found_index == expected_index) {
                    // If the filename ends with CAPTURE_META_ENDING, add it to
                    // the list of capture data files.
    ...
    // For each file found, read the capture into memory and add it to
    // finished_captures.
    DataCapture capture;
    for(std::vector<std::string>::iterator it = capture_files.begin();
            it != capture_files.end(); ++it) {
        capture.read_from_file(*it);
        state.finished_captures.push_back(capture);
    }
    // Sort finished_captures by the time they happened, with older captures
    // at the beginning.
    std::sort(state.finished_captures.begin(), state.finished_captures.end(),
            compare_captures);
}