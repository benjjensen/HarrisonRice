...
char *reg_names[32] = { 
	"PIO_OFFSET_CONTROL",
	"PIO_OFFSET_BLOCKSIZE",
	"PIO_OFFSET_NUMBLOCKS",
	"PIO_OFFSET_PCIE_RD_START_BLOCK",
	"PIO_OFFSET_ADCSERIAL",
	"PIO_OFFSET_PROMPROG",
	"PIO_OFFSET_CAPTUREADJ",
	"PIO_OFFSET_DACSERIAL",
	"PIO_OFFSET_ADC_DAC_SERIAL",
	"PIO_OFFSET_DAC_RD_START_BLOCK",
	"PIO_OFFSET_PCIE_RD_END_BLOCK",
	"PIO_OFFSET_DAC_RD_END_BLOCK",
	"PIO_OFFSET_RS_SERIAL_WR",
	"PIO_OFFSET_DRAM_ADJUST",
	"PIO_OFFSET_DAC_WR_START_BLOCK",
	"PIO_OFFSET_DAC_WR_END_BLOCK",
	"PIO_OFFSET_ADC_DATA_SETUP",
	"REG_11",
	"REG_12",
	"PIO_OFFSET_AVERAGER_CYCLES",
	"PIO_OFFSET_TRIGGER_THRESHOLD",
	"PIO_OFFSET_PRE_TRIGGER",
	"PIO_OFFSET_ISLA_CAPTURE",
	"PIO_OFFSET_AVERAGER_LEN",
	"REG_18",
	"REG_19",
	"PIO_OFFSET_ECL_TRIGGER_DELAY",
	"PIO_OFFSET_12BIT_GLOBAL_IDELAY",
	"PIO_OFFSET_CAPTURE_DEPTH",
	"PIO_OFFSET_CAPTURE_COUNT",
	"PIO_OFFSET_SYNTH_REG",
	"PIO_OFFSET_FIRMWARE_VERSION_RD"
};
...
{
	printf("If device does not run properly, then warm reset the system before running this software. Rarely required and then only for 110T/155T devices.\n"); 
	...
	for (unsigned short i = 0; i < ndevices; ++i)
	{	// for each device
	...
	i=0;
	printf("ApiInitialize() NumDevices = %d  \n", NumDevices);
	BoardSerialNum[i] = LoadSvf(i,"get_usercode.svf", 0);
	printf("BoardNum[%d] = %d\n",i,BoardSerialNum[i]);
...
short CDllDriverApi::ApiGetSerialNumber(unsigned short board_index)
{
	...
	if (board_index < MAX_DEVICES)
	{
		...
		if (serialNumber < 0)
			serialNumber = LoadSvf(board_index,"get_usercode.svf", 0);
	}
	return serialNumber;
